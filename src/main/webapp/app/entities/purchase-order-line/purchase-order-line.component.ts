...
export class PurchaseOrderLineComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.purchaseOrderLineService
            ...
            .subscribe(
                (res: HttpResponse<IPurchaseOrderLine[]>) => this.paginatePurchaseOrderLines(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/purchase-order-line'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: IPurchaseOrderLine) {
        return item.id;
    }

    registerChangeInPurchaseOrderLines() {
        this.eventSubscriber = this.eventManager.subscribe('purchaseOrderLineListModification', response => this.loadAll());
    }
    ...
    private paginatePurchaseOrderLines(data: IPurchaseOrderLine[], headers: HttpHeaders) {
